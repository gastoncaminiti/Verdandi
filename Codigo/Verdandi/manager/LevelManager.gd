...
signal units_centered

func _ready():
	emit_signal("units_centered",size_cell_x, size_cell_y)
...
func select_path():
	...
	print(path)
	var tl = map.map_to_world(map.world_to_map (get_node("Norse-Swordman").get_east_postion()))
	path[path.size()-1].x = tl.x + size_cell_x
